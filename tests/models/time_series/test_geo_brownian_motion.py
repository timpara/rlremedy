...
from rlremedy.models.time_series import geo_brownian_motion_utils
from rlremedy.models.time_series.univariate_brownian_motion import GeometricBrownianMotion,_sqrt_no_nan
from rlremedy.models.time_series.multivariate_geo_brownian_motion import MultivariateGeometricBrownianMotion
from utils.math import piecewise
from utils.math.random_ops import RandomType
arrays_all_close = geo_brownian_motion_utils.arrays_all_close
calculate_sample_paths_mean_and_variance = geo_brownian_motion_utils.calculate_sample_paths_mean_and_variance
...
class GeometricBrownianMotionTest(parameterized.TestCase, tf.test.TestCase):
  ...
  def test_univariate_constant_drift_and_volatility(self, dtype):
    ...
    drift_in = 0.05
    vol_in = 0.5
    process = GeometricBrownianMotion(drift_in, vol_in, dtype=dtype)
  ...
  def test_univariate_time_varying_drift_and_volatility(self, dtype):
    ...
    drift_in = piecewise.PiecewiseConstantFunc(
        jump_locations=times, values=drift, dtype=dtype)
    sigma_in = piecewise.PiecewiseConstantFunc(
  ...
  def test_univariate_integrate_parameter(self, dtype):
    ...
    time_varying_fn = piecewise.PiecewiseConstantFunc(
  ...
  def test_univariate_sample_mean_and_variance_time_varying_drift(
    ...
    with self.subTest('Drift as a step function, sigma = 0.0'):
      ...
      mu = piecewise.PiecewiseConstantFunc(jump_locations=mu_times,
                                                    values=mu_values,
                                                    dtype=dtype)
      sigma = 0.0
    ...
    with self.subTest('Drift = 0.05, sigma = step function'):
      ...
      sigma = piecewise.PiecewiseConstantFunc(
  ...
  def test_multivariate_sample_mean_and_variance(self, supply_draws, dtype):
    ...
    samples = process.sample_paths(
        ...
        random_type=RandomType.STATELESS,seed=[1,2],
    ...
    with self.subTest('Variance'):
      self.assertAllClose(tf.squeeze(var), expected_var, atol=1e-2, rtol=1e-2)
  ...
  def test_univariate_xla_compatible(self):
    ...
    process = GeometricBrownianMotion(0.05, 0.5, dtype=tf.float64)
    @tf.function
    def sample_fn():
